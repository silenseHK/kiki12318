<aside class="app-sidebar">
    <div class="app-sidebar__user">
        <div class="dropdown">
            <a class="nav-link pl-2 pr-2 leading-none d-flex" data-toggle="dropdown" href="#">
                <img alt="image" src="/admin/assets/img/avatar/avatar-1.jpeg.jpg" class=" avatar-md rounded-circle">
    ...
    <ul class="side-menu">
        ...
        <li class="slide">
            <a class="side-menu__item"  @if(!empty($val['child']))data-toggle="slide"@endif href="{{$val['url']}}"><i class="side-menu__icon fa {{$val['icon']}}"></i><span class="side-menu__label">{{$val['name']}}</span>@if(!empty($val['child']))<i class="angle fa fa-angle-right"></i>@endif</a>
            @if(!empty($val['child']))
            <ul class="slide-menu">
                @foreach($val['child'] as $k => $v)
                <li><a class="slide-item"  href="{{$v['url']}}"><span>{{$v['name']}}</span></a></li>
                @endforeach
            </ul>
            @endif
        </li>
